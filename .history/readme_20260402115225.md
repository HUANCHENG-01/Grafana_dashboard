Prometheus + Node Exporter 监控部署指南

本文档提供两种部署方案：Linux二进制部署（推荐生产环境）和Docker快速部署（适合测试/开发），并包含Grafana可视化配置指南。
...
2. 在 Query 标签页，选择 Prometheus 数据源，输入你的监控查询（如 CPU 使用率）：
   ...
   > 💡 说明：  
   > - CPU 使用率需要按 instance 拆分查询，否则会聚合所有服务器的 CPU 使用率，无法区分。  
   > - 内存使用率的查询语句利用了指标的 instance 标签分组特性，一条语句就能自动按服务器分组计算，因此可以同时展示多个服务器的内存使用率。
...
3. 在 「Regex」 中输入匹配原图例名称的正则（如 .instance="129.204.xx.xx".，或直接匹配默认图例）。
4. 在 「Replacement」 中输入新的名称（如 服务器A）。
5. 点击 「Apply」，图例名称会被批量替换。

5. 复用已有面板（可选）

Grafana 社区有大量现成的 Prometheus 监控面板，可直接导入使用（如 ID: 1860、8919 等），节省配置时间。

✅ 部署完成！你现在拥有一个完整的 Prometheus + Node Exporter + Grafana 监控体系。

如需扩展监控其他服务（如 MySQL、Redis、Kubernetes），可继续添加对应 Exporter 并配置 Prometheus 抓取。